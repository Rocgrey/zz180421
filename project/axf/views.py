...
from axf.models import SlideShow,Maindescription,Product,CategoriesGroup,ChildGroup,User,Address,Cart,Order
import random
...
def market(request, gid, cid, sid):
    #左侧分组数据
    leftCategorieList = CategoriesGroup.objects.all()
...
def product(request,pid):
    ...
    print(pro.name)
    return render(request, "market/product.html", {"pro":pro})
...
def login(request):
    if request.method == "GET":
        if request.is_ajax():
            ...
            print("********"+rand_str)
            response=JsonResponse({"data":"loginok"})
            return response
        else:
...
def adress(request):
    phone = request.session.get("phoneNum", default="未登录")
    return render(request, "mine/adress.html",{"phone":phone})
...
# def address(request):
#     if request.method=="GET":
#         return render(request,"mine/showAddress.html")